...
class ClpVarnishCacheAdmin {
    ...
    public function check_entire_cache_purge() {
        ...
        // Use sanitize_key() — nonces are opaque tokens, not general text fields
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce(sanitize_key($_GET['_wpnonce']), 'purge-entire-cache')) {
            return;
        }

        // Use home_url() instead of HTTP_HOST to prevent host-header spoofing
    ...
    public function add_adminbar($adminbar) {
        // Restrict admin bar menu to users who can actually trigger purges
    ...
    public static function get_svg_icon($base64 = true) {
        static $cache = [];
        $key = $base64 ? 'b64' : 'raw';
        if (isset($cache[$key])) {
            return $cache[$key];
        ...
        $cache['raw'] = $svg;
        $cache['b64'] = 'data:image/svg+xml;base64,' . base64_encode($svg);
        return $cache[$key];
    }
}
